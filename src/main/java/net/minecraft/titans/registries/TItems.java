...
import net.minecraft.titans.items.*;
...
public class TItems {
	public static RegistryEvent.Register<Item> registry;
	...
	public static final ItemMaterial toolMaterialOmni = new ItemMaterial("omni", Integer.MAX_VALUE, Integer.MAX_VALUE, Float.MAX_VALUE, Float.MAX_VALUE, 256);
	...
	public static void addEgg(String entityName, int primary) {
		for (int i = 0; i < 8; i++) {
			int secondary;
			switch (i) {
				case 1:
	...
	private static void add(String name, Item item, CreativeTabs tab, boolean usePrefix, String prefix) {
		add(name, null, item, tab, usePrefix, prefix);
	}

	private static void add(String name, String ore_dict_name, Item item, CreativeTabs tab) {
		add(name, ore_dict_name, item, tab, false, (String) null);
	}
	
	private static void add(String name, String ore_dict_name, Item item, CreativeTabs tab, boolean usePrefix, String prefix)
	{
		if (registry != null)
		{
			...
			if (FMLCommonHandler.instance().getEffectiveSide() == Side.CLIENT) {
				if (usePrefix)
					ModelLoader.setCustomModelResourceLocation(item, 0, new ModelResourceLocation(prefix, "inventory"));
				else
